  ...
  <div class="box-inner" style="background-color:#ffffff;">
    ...
    <div class="box-content" style="padding-bottom:50px;">
      ...
      <table class="table table-condensed">
        ...
        <tbody>
          @forelse($rows as $row)
          <tr onclick="location.href='{{ route('status_operating_costs.show', $row->id) }}'">
            ...
            <td>
              ...
              <!-- <button class="btn btn-xs" onclick="location.href='{{ route('status_operating_costs.show', $row->id) }}'">보기</button> -->
              <button class="btn btn-xs btn-primary" onclick="location.href='{{ route('status_operating_costs.edit', $row->id) }}'">수정</button>
      ...
      <div style="text-align:right; margin-top:45px;">
        <div class="bg-light" style="padding-top:10px;">
          <form action="{{ route('status_operating_costs.import') }}" method="POST" enctype="multipart/form-data" class="form__upload">
            @csrf
...
<script>
      ...
      if (idsArr.length <= 0)
      {
        alert("삭제할 항목을 선택해 주세요.");
          ...
          if (confirm("정말로 선택된 항목을 삭제하시겠습니까?"))
          {
            ...
            $.ajax({
                url: "{{ route('status_operating_costs.multiple-delete') }}",
                type: 'DELETE',
                data: 'ids=' + strIds,
  ...
  $('.btn-down-example').on('click', function () {
    window.location.href = '/status_operating_costs/example';
  })
