...
<div class="box col-md-12">
  <div class="box-inner" style="background-color:#ffffff;">
    ...
    <div class="box-content" style="padding-bottom:50px;">
      @include('small_farmers.partial.search')
      <table class="table table-condensed">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ number_format($farmer->sum_acreage) }}</td>
            <td>{{ number_format($farmer->acreage1) }}</td>
            <td>{{ number_format($farmer->acreage2) }}</td>
            <td>{{ number_format($farmer->acreage3) }}</td>
            ...
            <td>
              ...
              <button class="btn btn-xs btn-primary" onclick="location.href='{{ route('small_farmers.edit', $farmer->id) }}'">수정</button>
          ...
          @empty
          <tr>
            <td colspan="14">항목이 존재하지 않습니다.</td>
          </tr>
          @endforelse
        </tbody>
      </table>
      ...
      <div style="float:right;">
        @if($schedule->is_allow)
        <button type="button" class="btn btn-sm btn-primary" onclick="location.href='{{ route('small_farmers.create') }}'">등록</button>
        @endif
        @if($farmers->total() > 0)
        <a href="{{ route('small_farmers.export',
...
<script>
  ...
  $('.btn-down-example').on('click', function () {
    window.location.href = '/small_farmers/example';
  })
