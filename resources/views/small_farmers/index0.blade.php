...
@section('content')
  ...
  <table class="table">
    ...
    <tr>
      ...
      <td>{{ $farmer->sum_acreage }}</td>
      <td>{{ $farmer->acreage1 }}</td>
      <td>{{ $farmer->acreage2 }}</td>
      <td>{{ $farmer->acreage3 }}</td>
      <td>{{ $farmer->remark }}</td>
      ...
      <td>
        ...
        <a href="{{ route('small_farmers.show', $farmer->id) }}" class="btn btn-sm btn-primary">보기</a>
  ...
  @if($schedule->is_allow)
  <a href="{{ route('small_farmers.create') }}" class="btn btn-sm btn-primary">추가</a>
  @endif
  ...
  <div class="container">
    <div class="bg-light">
      <form action="{{ route('small_farmers.import') }}" method="POST" enctype="multipart/form-data" class="form__article">
        @csrf
