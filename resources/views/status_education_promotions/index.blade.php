  ...
  <div class="box-inner" style="background-color:#ffffff;">
    ...
    <div class="box-content" style="padding-bottom:50px;">
      ...
      <table class="table table-condensed">
        ...
        <tbody>
          @forelse($rows as $row)
          <tr onclick="location.href='{{ route('status_education_promotions.show', $row->id) }}'">
      ...
      <div style="float:right;">
        @if (auth()->user()->is_input_allowed)
        <button type="button" class="btn btn-sm btn-primary" onclick="location.href='{{ route('status_education_promotions.create') }}'">등록</button>
        @endif
        @if($rows->total() > 0)
      ...
      <div style="text-align:right; margin-top:45px;">
        <div class="bg-light" style="padding-top:10px;">
          <form action="{{ route('status_education_promotions.import') }}" method="POST" enctype="multipart/form-data" class="form__upload">
            @csrf
...
<script>
      ...
      if (idsArr.length <= 0)
      {
        alert("삭제할 항목을 선택해 주세요.");
          ...
          if (confirm("정말로 선택된 항목을 삭제하시겠습니까?"))
          {
            ...
            $.ajax({
                url: "{{ route('status_education_promotions.multiple-delete') }}",
                type: 'DELETE',
                data: 'ids=' + strIds,
  ...
  $('.btn-down-example').on('click', function () {
    window.location.href = '/status_education_promotions/example';
  })
