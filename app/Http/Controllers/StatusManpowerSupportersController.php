...
class StatusManpowerSupportersController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $duplicated_items = $this->check_duplicate($business_year, $supporter->name, $supporter->birth, $job_start_date, $job_end_date);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($row->job_start_date->format('Y-m-d') != $job_start_date || $row->job_end_date->format('Y-m-d') != $job_end_date)
        {
            ...
            //            그런데 동일인이 여러 농가에 등록되어 있을수 있다. 예를들면, C라는 작업자가 A농협에도, B농협에도 등록되어 있는데,
            //            동일인이므로 중복날짜에 있으면 오류를 알려줘야 한다. ($supporter_name, $supporter_birth로 검색)
            $duplicated_items = $this->check_duplicate($business_year, $supporter->name, $supporter->birth, $job_start_date, $job_end_date, $id);
    ...
    private function check_duplicate($business_year, $supporter_name, $supporter_birth, $job_start_date, $job_end_date, $edit_id='')
    {
